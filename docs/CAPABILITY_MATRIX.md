...
| Research workflow | Molemo WorkBench v0.9 | Current boundary |
| --- | --- | --- |
...
| Guided plans and researcher approval | Implemented | Twelve concrete templates; Agent may propose and inspect, while only the local UI can approve execution |
...
| Scientific artifacts | Implemented | Literature evidence map, clinical-trial landscape, target-evidence matrix, variant evidence document, molecule, protein sequence/structure, pairwise alignment, BLAST hits, database record, FASTQ QC, PCA, volcano, heatmap, ranked genes, bar chart and hydropathy track |
...
| Process benchmark | Implemented | Deterministic v0.9 suite including real local BLAST+ and PyDESeq2 tasks, target/literature/clinical-trial/variant routing, approval boundary and artifacts |

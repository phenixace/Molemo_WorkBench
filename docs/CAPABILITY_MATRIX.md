...
| Research workflow | Molemo WorkBench v0.13 | Current boundary |
| --- | --- | --- |
...
| Guided plans and researcher approval | Implemented | Sixteen concrete templates; Agent may propose and inspect, while only the local UI can approve execution |
...
| Process benchmark | Implemented | Deterministic v0.13 suite including real local BLAST+, HMMER, PyDESeq2 and Scanpy/Leiden tasks, routing, approval boundary, provenance and artifacts |
...
| Single-cell RNA-seq exploration | Implemented | Bounded cell-by-gene raw-count CSV/TSV plus exact cell metadata; QC, CP10k/log1p, HVG, PCA, neighbors, UMAP, Leiden, cluster markers, `.h5ad` and provenance; no 10x/H5 input, doublet/ambient-RNA correction, integration, automatic cell typing, trajectory or donor-aware pseudobulk |

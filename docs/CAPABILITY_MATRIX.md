...
| Research workflow | Molemo WorkBench v0.18 | Current boundary |
| --- | --- | --- |
...
| Guided plans and researcher approval | Implemented | Eighteen concrete templates; Agent may propose and inspect, while only the local UI can approve execution |
...
| Process benchmark | Implemented | Deterministic v0.18 suite including real local BLAST+, HMMER, PyDESeq2 and Scanpy/Scrublet/Leiden tasks, routing, approval boundary, provenance and artifacts |
...
| Public biological databases | Implemented | Source-linked ChEMBL, Europe PMC, ClinicalTrials.gov, ClinVar, Ensembl VEP, gnomAD, PubChem, UniProtKB, RCSB, AlphaFold DB, Open Targets, Reactome and STRING retrieval through fixed official hosts |

...
| Research workflow | Molemo WorkBench v0.8 | Current boundary |
| --- | --- | --- |
...
| Guided plans and researcher approval | Implemented | Eleven concrete templates; Agent may propose and inspect, while only the local UI can approve execution |
...
| Scientific artifacts | Implemented | Literature evidence map, target-evidence matrix, variant evidence document, molecule, protein sequence/structure, pairwise alignment, BLAST hits, database record, FASTQ QC, PCA, volcano, heatmap, ranked genes, bar chart and hydropathy track |
...
| Process benchmark | Implemented | Deterministic v0.8 suite including real local BLAST+ and PyDESeq2 tasks, target/literature/variant routing, approval boundary and artifacts |
...
| Public biological databases | Implemented | Source-linked Europe PMC, ClinVar, Ensembl VEP, gnomAD, PubChem, UniProtKB, RCSB and Open Targets retrieval through fixed official hosts |

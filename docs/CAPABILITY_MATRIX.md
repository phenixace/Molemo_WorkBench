...
| Guided plans and researcher approval | Implemented | Twenty-three concrete templates; Agent may propose and inspect, while only the local UI can approve execution |
...
| Process benchmark | Implemented | Deterministic v0.22 suite including real local BWA/samtools/bcftools, BLAST+, HMMER, MAFFT, PyDESeq2 and Scanpy/Scrublet/Leiden tasks, routing, approval boundary, provenance and artifacts |
...
| GEO Series Matrix import | Implemented | Exact GSE and official matrix file; approval-gated bounded gzip transfer, numeric/dimension validation, contact-metadata omission, local matrix/QC/provenance artifacts; processed values are not treated as raw counts and no supplementary/raw data, batch assessment or downstream DE is performed |
...
| Bounded DNA read-to-variant workflow | Implemented | Paired FASTQ plus a small FASTA reference; approval-gated BWA-MEM, sorted/indexed BAM, samtools QC/coverage and bcftools candidate VCF with provenance; synthetic validation only, with no production-scale reference, recalibration, cohort calling, annotation or clinical interpretation |
| Production WGS/WES | Planned | Requires reference bundles, interval handling, scalable execution, production caller/QC policy, annotation, cohort validation and operational monitoring |
| FASTQ-to-expression | Planned | Current bulk and single-cell workflows start from count matrices; no trimming, alignment/pseudoalignment or gene/transcript quantification |
| Metagenomics | Planned | No taxonomic profiling, assembly, binning or functional profiling workflow |
| Proteomics | Planned | No raw mass-spectrometry search, identification, FDR control or quantification workflow |

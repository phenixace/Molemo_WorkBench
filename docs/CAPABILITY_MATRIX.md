...
| Research workflow | Molemo WorkBench v0.11 | Current boundary |
| --- | --- | --- |
...
| Guided plans and researcher approval | Implemented | Fourteen concrete templates; Agent may propose and inspect, while only the local UI can approve execution |
...
| Process benchmark | Implemented | Deterministic v0.11 suite including real local BLAST+ and PyDESeq2 tasks, VCF/target/literature/clinical-trial/variant routing, approval boundary and artifacts |
...
| NGS analysis workbench | Partial | FASTQ QC, processed multi-sample VCF review and raw-count bulk RNA-seq DE implemented with preflight, approval, visualization and saved outputs; no FASTQ-to-count, raw-read variant calling or single-cell pipeline |

...
| Research workflow | Molemo WorkBench v0.7 | Current boundary |
| --- | --- | --- |
...
| Guided plans and researcher approval | Implemented | Ten concrete templates; Agent may propose and inspect, while only the local UI can approve execution |
...
| Scientific artifacts | Implemented | Literature evidence map, target-evidence matrix, molecule, protein sequence/structure, pairwise alignment, BLAST hits, database record, FASTQ QC, PCA, volcano, heatmap, ranked genes, bar chart and hydropathy track |
...
| Process benchmark | Implemented | Deterministic v0.7 suite including real local BLAST+ and PyDESeq2 tasks, target/literature routing, approval boundary and artifacts |
...
| Public biological databases | Implemented | Source-linked Europe PMC, PubChem, UniProtKB, RCSB and Open Targets retrieval through fixed official hosts |

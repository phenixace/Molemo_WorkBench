...
`single_cell.py` is the processed single-cell boundary. It validates bounded cell-by-gene raw integer counts, exact optional cell metadata, dataset-specific QC thresholds and the project Scanpy/Leiden runtime. After approval it invokes `tools/run_scanpy.py` without a shell and with fixed CPU and random-seed settings. The runner calculates cell/gene QC, CP10k/log1p normalization, highly variable genes, PCA, neighbors, UMAP, Leiden clusters and descriptive Wilcoxon markers, then atomically publishes TSV tables, `.h5ad`, package versions, source hashes, manifest and summary under `workspace/analyses/`. It performs no cell-type naming, doublet or ambient-RNA correction, batch integration, trajectory inference or donor-aware differential expression.

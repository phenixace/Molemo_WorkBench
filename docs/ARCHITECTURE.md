...
`dna_variant_calling.py` validates synchronized paired FASTQ, a bounded FASTA reference, sample identity, resource limits, and local BWA/samtools/bcftools versions before approval. Approved execution runs fixed argument vectors without a shell, streams BWA-MEM into coordinate sorting, indexes the BAM, records mapping and coverage metrics, and generates a normalized unfiltered candidate VCF. BAM/BAI, VCF, coverage, call tables, summaries, versions, hashes, and a manifest are atomically published under `workspace/analyses/`. This lane is a small-reference systems demonstration, not a production WGS/WES or clinical caller.

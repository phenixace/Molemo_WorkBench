...
| Guided plan 与研究者审批 | 已实现 | 二十二类固定模板；Agent 可提出和检查，只有本地 UI 可以批准执行 |
...
| 科学 artifacts | 已实现 | GEO 版图/Matrix QC、结构、序列、BLAST、HMMER、MAFFT、通路/网络、文献、临床试验、VCF、单细胞、PCA、volcano、heatmap 等 typed artifacts |
| 本地文件工作区 | 已实现 | 显式上传、Agent 只读、路径限制；批准后的 pipeline 可以写入有界输出 |
| 可审计运行记录 | 已实现 | 计划输入、逐步状态、工具参数、时间、摘要、对话与 artifacts |
| 过程评测 | 已实现 | v0.22 确定性测试覆盖真实 BLAST+、HMMER、MAFFT、PyDESeq2、Scanpy、路由、审批、provenance 与 artifacts |
...
| NGS 分析工作台 | 部分实现 | FASTQ QC、processed VCF、raw-count bulk DE 和 processed single-cell 已有审批、可视化与保存输出；稳定版尚无 FASTQ-to-count 或 raw-read variant calling |

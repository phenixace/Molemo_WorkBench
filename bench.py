...
def cleanup_task_output(task: dict[str, Any], result: dict[str, Any] | None) -> None:
    ...
    if target.parent == analyses_root and target.name.startswith(
        ("rnaseq-", "hmmer-search-", "single-cell-", "functional-analysis-")
    ):
        shutil.rmtree(target, ignore_errors=True)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Molemo_Bench v0.15 against the local skill registry.")
